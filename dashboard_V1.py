...
@st.cache_data
def load_data():
    ref = db.reference("/dados/DHT11_01")
    data = ref.get()
    if not data:
        return pd.DataFrame()
    rows = []
...
if not df.empty:
    ...
    fig, ax = plt.subplots()
    ax.plot(df_filtered["timestamp"], df_filtered["temperature"], marker="o", linestyle="-", color="b")
    ...
    fig, ax = plt.subplots()
    ax.plot(df_filtered["timestamp"], df_filtered["humidity"], marker="o", linestyle="-", color="g")
    ax.set_xlabel("Timestamp")
    ...
    fig, ax = plt.subplots()
    ax.plot(df_filtered["timestamp"], df_filtered["uv"], marker="o", linestyle="-", color="purple")
    ax.set_xlabel("Timestamp")
    ax.set_ylabel("UV (mW/cm²)")
    ax.set_title("UV ao Longo do Tempo")
    ax.grid(True)
    plt.xticks(rotation=45)
    st.pyplot(fig)
    ...
    if alerts and isinstance(alerts, dict):
        ...
        st.dataframe(pd.DataFrame(alert_rows)[["timestamp", "sensor_id", "reason", "temperature", "uv"]])
    else:
